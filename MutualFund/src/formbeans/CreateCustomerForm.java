...
public class CreateCustomerForm extends FormBean {
	...
	public List<String> getValidationErrors() {
		...
		if (userName.matches(".*[<>\"].*") || firstName.matches(".*[<>\"].*")
				|| lastName.matches(".*[<>\"].*")) {
			errors.add("Name may not contain angle brackets or quotes");
		}
		if (!dataConversion.validStringLength(userName)
				|| !dataConversion.validStringLength(firstName)
				|| !dataConversion.validStringLength(lastName)) {
		...
		if (!dataConversion.validStringLength(address1)
				|| !dataConversion.validStringLength(address2)) {
			errors.add("Address should be no longer than 30 characters");
		}
